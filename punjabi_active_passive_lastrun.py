...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\Punjabi\\punjabi_active_passive_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    #Show image for 5 min
    image_time = 5
    movie_time = 4
    sentence_time = audio_time = 4
    ...
    tr1_text = "Session 1\n\n" + tr1_text_base + "\n" + keypress_text
    tr2_text_base = (
        "Please look at the fixation cross on the screen.\n"
        "Listen to the sentence.\n"
        "After that the fixation cross hair will reappear.\n"
        "Look at the fixation cross till the image appears.\n"
        "Look at the image as long as it is on the screen.\n\n"
    )
    ...
    # Compute the list index based on p_id
    list_index = p_id % 6
    
    # Select the appropriate slice based on list_index
    if list_index == 0:
        list_selector = list1
    elif list_index == 1:
        list_selector = list2
    ...
    filename = "experiment_log_pujabi.txt"
    ...
    # Append new entry
    new_entry = f"{p_id}, {current_time}, {list_index}\n"
    ...
    print(f"✅ Experiment {p_id} saved at {current_time}")
    ...
    break_text = visual.TextStim(win=win, name='break_text',
        text='You have completed Session 1\n\n',
        font='Times New Roman',
        units='height', pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color=[-1.0000, -1.0000, -1.0000], colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
    ...
    s1_crosshair_s = visual.TextStim(win=win, name='s1_crosshair_s',
        text='+',
        font='Arial',
        units='height', pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color='black', colorSpace='rgb', opacity=None, 
        languageStyle='LTR',
        depth=-1.0);
    s1_sentence = visual.ImageStim(
        win=win,
        name='s1_sentence', units='pix', 
        image='default.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(1920, 1080),
    ...
    # --- Initialize components for Routine "S1_inter_trial_interval" ---
    blank_iti_2 = visual.TextStim(win=win, name='blank_iti_2',
        text='',
        font='Arial',
        units='height', pos=(0, 0), draggable=False, height=0.05, wrapWidth=None, ori=0.0, 
        color=[-1.0000, -1.0000, -1.0000], colorSpace='rgb', opacity=None, 
    ...
    textbox = visual.TextBox2(
         win, text='Thank you for your participation.', placeholder='Type here...', font='Times New Roman',
         ori=0.0, pos=(0, 0), draggable=False, units='height',     letterHeight=0.05,
         size=(2, 2), borderWidth=2.0,
         color=[-1.0000, -1.0000, -1.0000], colorSpace='rgb',
         opacity=None,
    ...
    for thisFree_view_loop in free_view_loop:
        ...
        TR1_eyelink_start = data.Routine(
            ...
            components=[HostDrawing, StartRecord],
        )
        ...
        el_tracker.sendCommand('clear_screen 0')
        # imagesAndComponentsStringList value = ['image,tr1_image']
        ...
        imagesAndComponentsListForHostBackdrop = [[image, tr1_image]]
        ...
        # Viewer User Manual, "Protocol for EyeLink Data to Viewer Integration"
        trialConditionVariablesForEyeLinkLogging = [image_id,image,stim_id,voice_id,session,filename,voice]
        trialConditionVariableNamesForEyeLinkLogging = ['image_id', 'image', 'stim_id', 'voice_id', 'session', 'filename', 'voice']
        ...
        while continueRoutine:
            ...
            if tr1_image.status == NOT_STARTED and tThisFlip >= fixDur-frameTolerance:
                # keep track of start time/frame for later
            ...
            # if tr1_image is stopping this frame...
            if tr1_image.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > tr1_image.tStartRefresh + image_time-frameTolerance:
                    # keep track of stop time/frame for later
                    tr1_image.tStop = t  # not accounting for scr refresh
                    tr1_image.tStopRefresh = tThisFlipGlobal  # on global time
                    tr1_image.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'tr1_image.stopped')
                    # update status
                    tr1_image.status = FINISHED
                    tr1_image.setAutoDraw(False)
        ...
        msg_completed = f'{free_view_loop.thisN + 1}/{free_view_loop.nTotal} Completed'
        blank_iti.setText(msg_completed
        )
        ...
        while continueRoutine and routineTimer.getTime() < 3.0:
            ...
            if blank_iti.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > blank_iti.tStartRefresh + 3-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if inter_trial_interval.maxDurationReached:
            routineTimer.addTime(-inter_trial_interval.maxDuration)
        elif inter_trial_interval.forceEnded:
            routineTimer.reset()
        else:
    ...
    while continueRoutine and routineTimer.getTime() < 4.0:
        ...
        if break_text.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > break_text.tStartRefresh + 4-frameTolerance:
                # keep track of stop time/frame for later
    ...
    # set up handler to look after randomisation of conditions etc
    active_passive = data.TrialHandler2(
        name='active_passive',
        nReps=1.0, 
        method='sequential', 
        extraInfo=expInfo, 
        ...
        'exp_files/list_selector.csv', 
        selection=list_selector
    )
    , 
        seed=None, 
    )
    thisExp.addLoop(active_passive)  # add the loop to the experiment
    thisActive_passive = active_passive.trialList[0]  # so we can initialise stimuli with some values
    # abbreviate parameter names if possible (e.g. rgb = thisActive_passive.rgb)
    if thisActive_passive != None:
        for paramName in thisActive_passive:
            globals()[paramName] = thisActive_passive[paramName]
    
    for thisActive_passive in active_passive:
        currentLoop = active_passive
        thisExp.timestampOnFlip(win, 'thisRow.t', format=globalClock.format)
        # abbreviate parameter names if possible (e.g. rgb = thisActive_passive.rgb)
        if thisActive_passive != None:
            for paramName in thisActive_passive:
        ...
        experiment_loop = data.TrialHandler2(
            ...
            extraInfo=expInfo, 
            originPath=-1, 
            trialList=data.importConditions(file_csv_selected), 
            seed=None, 
        )
        ...
        for thisExperiment_loop in experiment_loop:
            ...
            S1_eyelink_start = data.Routine(
                ...
                components=[HostDrawing_2, StartRecord_2],
            )
            ...
            # clear the host screen before we draw the backdrop
            el_tracker.sendCommand('clear_screen 0')
            # imagesAndComponentsStringList value = ['image,s1_image', 'sentence,s1_sentence']
            # Send image components to the Host PC backdrop to serve as landmarks during recording
            # The method bitmapBackdrop() requires a step of converting the
            # image pixels into a recognizable format by the Host PC.
            # pixels = [line1, ...lineH], line = [pix1,...pixW], pix=(R,G,B)
            ...
            # parameters: width, height, pixel, crop_x, crop_y,
            #             crop_width, crop_height, x, y on the Host, drawing options
            imagesAndComponentsListForHostBackdrop = [[image, s1_image], [sentence, s1_sentence]]
            # get the array of blank pixels where each pixel corresponds to win.color
            pixels = blankHostPixels[::]
            # go through each image and replace the pixels in the blank array with the image pixels
            for thisImage in imagesAndComponentsListForHostBackdrop:
            ...
            S1_image_display = data.Routine(
                ...
                components=[s1_crosshair_s, s1_sentence, sound_1, s1_crosshair, s1_image, MarkEvents_2],
            )
            ...
            s1_sentence.setImage(sentence)
            sound_1.setSound(audio, secs=audio_time, hamming=True)
            ...
            # log trial variables' values to the EDF data file, for details, see Data
            # Viewer User Manual, "Protocol for EyeLink Data to Viewer Integration"
            trialConditionVariablesForEyeLinkLogging = [voice_id,image_id,image,stim_id,sentence,audio,session,filename,voice]
            trialConditionVariableNamesForEyeLinkLogging = ['voice_id', 'image_id', 'image', 'stim_id', 'sentence', 'audio', 'session', 'filename', 'voice']
            for i in range(len(trialConditionVariablesForEyeLinkLogging)):
                el_tracker.sendMessage('!V TRIAL_VAR %s %s'% (trialConditionVariableNamesForEyeLinkLogging[i],trialConditionVariablesForEyeLinkLogging[i]))
                #add a brief pause after every 5 messages or so to make sure no messages are missed
                if i % 5 == 0:
            ...
            # list of all stimulus components whose onset/offset will be marked with messages
            componentsForEyeLinkStimEventMessages = [s1_crosshair,s1_sentence,s1_crosshair_s,s1_image]
            # list of all stimulus components for which Data Viewer draw commands will be sent
            componentsForEyeLinkStimDVDrawingMessages = [s1_crosshair,s1_sentence,s1_crosshair_s,s1_image]
            # list of all stimulus components which will have interest areas automatically created for them
            ...
            while continueRoutine:
                ...
                if t >= 2*fixDur+image_time+movie_time:
                    continueRoutine = False
                
                # *s1_crosshair_s* updates
                
                # if s1_crosshair_s is starting this frame...
                if s1_crosshair_s.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    s1_crosshair_s.frameNStart = frameN  # exact frame index
                    s1_crosshair_s.tStart = t  # local t and not account for scr refresh
                    s1_crosshair_s.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(s1_crosshair_s, 'tStartRefresh')  # time at next scr refresh
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 's1_crosshair_s.started')
                    # update status
                    s1_crosshair_s.status = STARTED
                    s1_crosshair_s.setAutoDraw(True)
                
                # if s1_crosshair_s is active this frame...
                if s1_crosshair_s.status == STARTED:
                    # update params
                    pass
                
                # if s1_crosshair_s is stopping this frame...
                if s1_crosshair_s.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > s1_crosshair_s.tStartRefresh + fixDur-frameTolerance:
                        # keep track of stop time/frame for later
                        s1_crosshair_s.tStop = t  # not accounting for scr refresh
                        s1_crosshair_s.tStopRefresh = tThisFlipGlobal  # on global time
                        s1_crosshair_s.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 's1_crosshair_s.stopped')
                        # update status
                        s1_crosshair_s.status = FINISHED
                        s1_crosshair_s.setAutoDraw(False)
                
                # *s1_sentence* updates
                
                # if s1_sentence is starting this frame...
                if s1_sentence.status == NOT_STARTED and tThisFlip >= fixDur-frameTolerance:
                    # keep track of start time/frame for later
                    s1_sentence.frameNStart = frameN  # exact frame index
                    s1_sentence.tStart = t  # local t and not account for scr refresh
                    s1_sentence.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(s1_sentence, 'tStartRefresh')  # time at next scr refresh
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 's1_sentence.started')
                    # update status
                    s1_sentence.status = STARTED
                    s1_sentence.setAutoDraw(True)
                
                # if s1_sentence is active this frame...
                if s1_sentence.status == STARTED:
                    # update params
                    pass
                
                # if s1_sentence is stopping this frame...
                if s1_sentence.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > s1_sentence.tStartRefresh + sentence_time-frameTolerance:
                        # keep track of stop time/frame for later
                        s1_sentence.tStop = t  # not accounting for scr refresh
                        s1_sentence.tStopRefresh = tThisFlipGlobal  # on global time
                        s1_sentence.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 's1_sentence.stopped')
                        # update status
                        s1_sentence.status = FINISHED
                        s1_sentence.setAutoDraw(False)
                
                # *sound_1* updates
                
                # if sound_1 is starting this frame...
                if sound_1.status == NOT_STARTED and t >= fixDur-frameTolerance:
                    # keep track of start time/frame for later
                ...
                if sound_1.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > sound_1.tStartRefresh + audio_time-frameTolerance or sound_1.isFinished:
                        # keep track of stop time/frame for later
                ...
                if s1_crosshair.status == NOT_STARTED and tThisFlip >= fixDur+sentence_time-frameTolerance:
                    # keep track of start time/frame for later
                ...
                if s1_image.status == NOT_STARTED and tThisFlip >= fixDur+sentence_time+fixDur-frameTolerance:
                    # keep track of start time/frame for later
                ...
                # if s1_image is stopping this frame...
                if s1_image.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > s1_image.tStartRefresh + image_time-frameTolerance:
                        # keep track of stop time/frame for later
                        s1_image.tStop = t  # not accounting for scr refresh
                        s1_image.tStopRefresh = tThisFlipGlobal  # on global time
                        s1_image.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 's1_image.stopped')
                        # update status
                        s1_image.status = FINISHED
                        s1_image.setAutoDraw(False)
            ...
            # --- Prepare to start Routine "S1_inter_trial_interval" ---
            # create an object to store info about Routine S1_inter_trial_interval
            S1_inter_trial_interval = data.Routine(
                name='S1_inter_trial_interval',
                components=[blank_iti_2],
            )
            S1_inter_trial_interval.status = NOT_STARTED
            continueRoutine = True
            # update component parameters for each repeat
            # Run 'Begin Routine' code from code_7
            msg_completed = f'Trial {(experiment_loop.thisN + 1)+8*(active_passive.thisN)}/{active_passive.nTotal*experiment_loop.nTotal} Completed'
            blank_iti_2.setText(msg_completed
            )
            # store start times for S1_inter_trial_interval
            S1_inter_trial_interval.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
            S1_inter_trial_interval.tStart = globalClock.getTime(format='float')
            S1_inter_trial_interval.status = STARTED
            thisExp.addData('S1_inter_trial_interval.started', S1_inter_trial_interval.tStart)
            S1_inter_trial_interval.maxDuration = None
            # keep track of which components have finished
            S1_inter_trial_intervalComponents = S1_inter_trial_interval.components
            for thisComponent in S1_inter_trial_interval.components:
                thisComponent.tStart = None
                thisComponent.tStop = None
                thisComponent.tStartRefresh = None
                thisComponent.tStopRefresh = None
            ...
            # --- Run Routine "S1_inter_trial_interval" ---
            # if trial has changed, end Routine now
            if isinstance(experiment_loop, data.TrialHandler2) and thisExperiment_loop.thisN != experiment_loop.thisTrial.thisN:
                continueRoutine = False
            S1_inter_trial_interval.forceEnded = routineForceEnded = not continueRoutine
            while continueRoutine and routineTimer.getTime() < 3.0:
                # get current time
                t = routineTimer.getTime()
                tThisFlip = win.getFutureFlipTime(clock=routineTimer)
                ...
                # if blank_iti_2 is starting this frame...
                if blank_iti_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                    blank_iti_2.frameNStart = frameN  # exact frame index
                    blank_iti_2.tStart = t  # local t and not account for scr refresh
                    blank_iti_2.tStartRefresh = tThisFlipGlobal  # on global time
                    win.timeOnFlip(blank_iti_2, 'tStartRefresh')  # time at next scr refresh
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'blank_iti_2.started')
                    # update status
                    blank_iti_2.status = STARTED
                    blank_iti_2.setAutoDraw(True)
                
                # if blank_iti_2 is active this frame...
                if blank_iti_2.status == STARTED:
                    # update params
                    pass
                
                # if blank_iti_2 is stopping this frame...
                if blank_iti_2.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > blank_iti_2.tStartRefresh + 3-frameTolerance:
                        # keep track of stop time/frame for later
                        blank_iti_2.tStop = t  # not accounting for scr refresh
                        blank_iti_2.tStopRefresh = tThisFlipGlobal  # on global time
                        blank_iti_2.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 'blank_iti_2.stopped')
                        # update status
                        blank_iti_2.status = FINISHED
                        blank_iti_2.setAutoDraw(False)
                if thisExp.status == FINISHED or endExpNow:
                    endExperiment(thisExp, win=win)
                    return
                # pause experiment here if requested
                ...
                # check if all components have finished
                if not continueRoutine:  # a component has requested a forced-end of Routine
                    S1_inter_trial_interval.forceEnded = routineForceEnded = True
                    break
                continueRoutine = False  # will revert to True if at least one component still running
                for thisComponent in S1_inter_trial_interval.components:
                    if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
            ...
            # --- Ending Routine "S1_inter_trial_interval" ---
            for thisComponent in S1_inter_trial_interval.components:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            # store stop times for S1_inter_trial_interval
            S1_inter_trial_interval.tStop = globalClock.getTime(format='float')
            S1_inter_trial_interval.tStopRefresh = tThisFlipGlobal
            thisExp.addData('S1_inter_trial_interval.stopped', S1_inter_trial_interval.tStop)
            # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
            if S1_inter_trial_interval.maxDurationReached:
                routineTimer.addTime(-S1_inter_trial_interval.maxDuration)
            elif S1_inter_trial_interval.forceEnded:
                routineTimer.reset()
            else:
                routineTimer.addTime(-3.000000)
            thisExp.nextEntry()
        ...
        experiment_loop.saveAsText(filename + 'experiment_loop.csv', delim=',',
            stimOut=params,
            dataOut=['n','all_mean','all_std', 'all_raw'])
    # completed 1.0 repeats of 'active_passive'
